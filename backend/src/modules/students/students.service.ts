...
export async function addSkill(studentId: string, skillName: string) {
  ...
  if (!trimmedName) throw ApiError.badRequest('Skill name cannot be empty');
  ...
  // Check if already exists (the @@unique([studentId, skillId]) will also catch it, but give a friendly error)
  ...
  if (existing) {
    throw ApiError.conflict(`Skill '${skill.canonicalName}' is already added`);
  }
  ...
  // Recalculate completeness after adding a skill (may now meet threshold)
  const completeness = await recalculateCompleteness(studentId);
  await prisma.student.update({
    where: { id: studentId },
    data: { profileCompleteness: completeness },
...
export async function removeSkill(studentId: string, skillId: string) {
  ...
  if (!evidence) throw ApiError.notFound('Skill evidence');
