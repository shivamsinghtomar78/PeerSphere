# Phase 8: AI Matching Engine — Fallback Strategy

## Objective

Document the implementation contract for **Fallback Strategy** within Phase 8. Defines rules-only operation, manual requirement confirmation, queued retries, circuit breakers, cached approved taxonomy, and clear degraded-service messages when AI services fail.

## Deliverables

- An approved, version-controlled specification for Fallback Strategy.
...
Defines rules-only operation, manual requirement confirmation, queued retries, circuit breakers, cached approved taxonomy, and clear degraded-service messages when AI services fail.
...
### Task 8.1: Define and implement Fallback Strategy
...
- Planned API: `apps/api/src/modules/fallback-strategy/`
- Planned web client: `apps/web/src/features/fallback-strategy/`
- Planned tests: `apps/api/test/fallback-strategy/` and `apps/web/src/features/fallback-strategy/__tests__/`
...
A developer can implement and verify Fallback Strategy using only this document and its linked architecture, security, data, and test contracts.
