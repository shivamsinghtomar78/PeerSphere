# Phase 8: AI Matching Engine — Explainability

## Objective

Document the implementation contract for **Explainability** within Phase 8. Defines structured matched, partial, missing, and unsupported requirements, weighted contributions, source evidence, model/rule versions, and plain-language caveats.

## Deliverables

- An approved, version-controlled specification for Explainability.
...
Defines structured matched, partial, missing, and unsupported requirements, weighted contributions, source evidence, model/rule versions, and plain-language caveats.
...
### Task 8.1: Define and implement Explainability
...
- Planned API: `apps/api/src/modules/explainability/`
- Planned web client: `apps/web/src/features/explainability/`
- Planned tests: `apps/api/test/explainability/` and `apps/web/src/features/explainability/__tests__/`
...
A developer can implement and verify Explainability using only this document and its linked architecture, security, data, and test contracts.
...
```json
{
  ...
  "matchedRequirements": [{"requirementId": "uuid", "evidence": [{"resumeVersionId": "uuid", "text": "…", "location": "p2"}], "contribution": 12}],
  "partialMatches": [],
  "missingRequirements": [],
  "irrelevantOrLowRelevanceSkills": [],
  "skillGaps": [],
  "explanation": "Evidence-backed, plain-language summary.",
  "recommendations": [],
...
Explanations must be generated from the structured result, not substituted for it. Display source evidence, distinguish missing evidence from missing ability, identify configuration/model versions to administrators, and state that the outcome supports review rather than a final hiring decision.
