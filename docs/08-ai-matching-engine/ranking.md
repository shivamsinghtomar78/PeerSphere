# Phase 8: AI Matching Engine — Ranking

## Objective

Document the implementation contract for **Ranking** within Phase 8. Defines stable sorting: eligibility state, review safety, final score, then deterministic tiebreakers. Rankings are scoped to a job version and recalculated asynchronously.

## Deliverables

- An approved, version-controlled specification for Ranking.
...
Defines stable sorting: eligibility state, review safety, final score, then deterministic tiebreakers. Rankings are scoped to a job version and recalculated asynchronously.
...
### Task 8.1: Define and implement Ranking
...
- Planned API: `apps/api/src/modules/ranking/`
- Planned web client: `apps/web/src/features/ranking/`
- Planned tests: `apps/api/test/ranking/` and `apps/web/src/features/ranking/__tests__/`
...
A developer can implement and verify Ranking using only this document and its linked architecture, security, data, and test contracts.
