# Phase 13: Security and Privacy — Privacy

## Objective

Document the implementation contract for **Privacy** within Phase 13. Defines data inventory, purpose limitation, consent/notice, minimum collection, access/correction/deletion workflow, retention schedule, processor review, and jurisdiction-specific legal review before launch.

## Deliverables

- An approved, version-controlled specification for Privacy.
...
Architecture, authentication, data handling, and operational decisions are documented.

## Implementation Requirements

Defines data inventory, purpose limitation, consent/notice, minimum collection, access/correction/deletion workflow, retention schedule, processor review, and jurisdiction-specific legal review before launch.
...
### Task 13.1: Define and implement Privacy
...
- Planned API: `apps/api/src/modules/privacy/`
- Planned web client: `apps/web/src/features/privacy/`
- Planned tests: `apps/api/test/privacy/` and `apps/web/src/features/privacy/__tests__/`
...
A developer can implement and verify Privacy using only this document and its linked architecture, security, data, and test contracts.
...
### Task 13.2: Review evidence and release gate
...
- `docs/13-security-and-privacy/phase-completion.md`
- Test reports and CI artifacts for the implementing services.

#### Dependencies

Task 13.1 has implementation and test evidence.
...
| Stage | Control |
|---|---|
| Collect | Collect only fields necessary for placement support; present notice and consent/appropriate lawful basis approved by the institution. |
| Store | Keep resumes in private encrypted object storage; keep references and minimal metadata in the database. |
| Use | Enforce role/ownership authorization; do not use production resumes for model training or evaluation without separately documented approval. |
| Share | Do not expose peer data; any export needs authorization, audit, and purpose review. |
| Retain | Configure institution-approved schedules for applications, audits, and resumes; publish them before launch. |
| Delete | Queue verified deletion from database, storage, caches, and search indexes where applicable; preserve only legally required audit metadata. |
| Respond | Support access, correction, deletion, and incident processes through a documented owner and SLA. |

Before deployment, have the institution/legal owner validate applicable privacy law, data-processing agreements, cross-border provider processing, student notices, and retention periods. This technical plan is not legal advice.
