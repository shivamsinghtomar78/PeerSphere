...
Document the implementation contract for **Schema** within Phase 3. Defines relational entities and ownership boundaries: users, students, education, skills, evidence, resumes, jobs, job requirements, evaluations, recommendations, applications, overrides, and audit events.

## Deliverables

- An approved, version-controlled specification for Schema.
...
Phase 2 architecture, data ownership, and API boundaries are approved.

## Implementation Requirements

Defines relational entities and ownership boundaries: users, students, education, skills, evidence, resumes, jobs, job requirements, evaluations, recommendations, applications, overrides, and audit events.
...
### Task 3.1: Define and implement Schema
...
- Planned API: `apps/api/src/modules/schema/`
- Planned web client: `apps/web/src/features/schema/`
- Planned tests: `apps/api/test/schema/` and `apps/web/src/features/schema/__tests__/`
...
A developer can implement and verify Schema using only this document and its linked architecture, security, data, and test contracts.
...
### Task 3.2: Review evidence and release gate
...
- `docs/03-database/phase-completion.md`
- Test reports and CI artifacts for the implementing services.

#### Dependencies

Task 3.1 has implementation and test evidence.
...
```text
users 1─1 students 1─* resume_versions 1─* resume_extractions
students 1─* student_skill_evidence ─* skills
jobs 1─* job_versions 1─* job_requirements ─* skills
job_versions × students → evaluations 1─* requirement_matches
evaluations 1─* recommendations
jobs × students → applications
users 1─* audit_events; evaluations 1─* review_overrides
```

| Table | Critical fields / constraints |
|---|---|
| users | id UUID, email unique, role enum, status, created_at. |
| students | user_id unique FK, branch, graduation_year, cgpa decimal(4,2) nullable, backlog_count nullable. |
| resume_versions | id, student_id FK, storage_key unique, checksum, state enum, parser_version, deleted_at. |
| skills | id, canonical_name unique, category, taxonomy_version, deprecated_at. |
| student_skill_evidence | student_id, skill_id, resume_version_id nullable, source_type, evidence_text/location, proficiency, confidence. |
| jobs/job_versions | owner, state, version unique per job, immutable published_at, source document pointer. |
| job_requirements | job_version_id, type HARD/SOFT, field/operator/value for hard rules, skill_id nullable, weight, requiredness, source span. |
| evaluations | student_id, job_version_id, input snapshot hash, eligibility state, match_score, confidence_score, classification, review flag, algorithm version, unique input snapshot. |
| requirement_matches | evaluation_id, requirement_id, match state, contribution, evidence IDs, explanation. |
| review_overrides | evaluation_id, actor_id, decision, reason, created_at; append-only. |
| audit_events | actor_id nullable, action, resource type/id, request ID, metadata JSONB redacted, timestamp. |

Use UUID primary keys, `created_at` and `updated_at` timestamps, transactional migrations, FK constraints, and database checks for 0–100 scores and valid workflow enums.
