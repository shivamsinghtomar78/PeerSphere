# Phase 9: Skill Gap Analysis — Resume Vs JD

## Objective

Document the implementation contract for **Resume Vs JD** within Phase 9. Defines per-requirement tables showing job text, requirement type, matched candidate evidence/location, match level, confidence, and a safe explanation.

## Deliverables

- An approved, version-controlled specification for Resume Vs JD.
...
Defines per-requirement tables showing job text, requirement type, matched candidate evidence/location, match level, confidence, and a safe explanation.
...
### Task 9.1: Define and implement Resume Vs JD
...
- Planned API: `apps/api/src/modules/resume-vs-jd/`
- Planned web client: `apps/web/src/features/resume-vs-jd/`
- Planned tests: `apps/api/test/resume-vs-jd/` and `apps/web/src/features/resume-vs-jd/__tests__/`
...
A developer can implement and verify Resume Vs JD using only this document and its linked architecture, security, data, and test contracts.
