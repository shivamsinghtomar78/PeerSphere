# Phase 6: Placement Cell Module — Filtering

## Objective

Document the implementation contract for **Filtering** within Phase 6. Defines server-side validated filters for eligibility, branch, year, CGPA, skills, score range, confidence, review state, application state, and job version.

## Deliverables

- An approved, version-controlled specification for Filtering.
...
Defines server-side validated filters for eligibility, branch, year, CGPA, skills, score range, confidence, review state, application state, and job version.
...
### Task 6.1: Define and implement Filtering
...
- Planned API: `apps/api/src/modules/filtering/`
- Planned web client: `apps/web/src/features/filtering/`
- Planned tests: `apps/api/test/filtering/` and `apps/web/src/features/filtering/__tests__/`
...
A developer can implement and verify Filtering using only this document and its linked architecture, security, data, and test contracts.
