...
Document the implementation contract for **Design System** within Phase 11. Defines semantic tokens, typography, color contrast, spacing, responsive rules, focus treatment, data-state components, and tested components for status, score, confidence, evidence, and warnings.

## Deliverables

- An approved, version-controlled specification for Design System.
...
Defines semantic tokens, typography, color contrast, spacing, responsive rules, focus treatment, data-state components, and tested components for status, score, confidence, evidence, and warnings.
...
### Task 11.1: Define and implement Design System
...
- Planned API: `apps/api/src/modules/design-system/`
- Planned web client: `apps/web/src/features/design-system/`
- Planned tests: `apps/api/test/design-system/` and `apps/web/src/features/design-system/__tests__/`
...
A developer can implement and verify Design System using only this document and its linked architecture, security, data, and test contracts.
...
Implement the design system with Next.js TypeScript components, Tailwind CSS utilities that consume semantic CSS variables, and modern CSS features with graceful fallbacks. Keep visual tokens in one theme layer; component-specific CSS must not invent an unreviewed color, spacing, or glass value. Motion must respect user preference and be optional for task completion.
